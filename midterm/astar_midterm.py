...
# originator Kerstin Voigt, for CSE 512, Winter 2019
...
def extract_path(nd,closed):
  temp = [nd]
  if nd.theparent is None:
    return temp
  traverseNode = nd.theparent
  temp = [traverseNode] + temp
  
  while traverseNode.theparent is not None:
    for x in closed:
      if traverseNode.thestate is x.thestate:
        traverseNode = traverseNode.theparent
    temp = [traverseNode] + temp

  return temp  # place holder; replace with non-None #************************************************************
